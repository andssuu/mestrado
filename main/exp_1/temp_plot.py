...
if __name__ == "__main__":
    ...
    for line in file.readlines():
        raw_ts, _, _, _, raw_temp, *_  = line.strip().split(';')
        ts.append(int(raw_ts))
        temp.append(float(raw_temp))
    ...
    for n in range(24):
        ...
        axs[x_plot, y_plot].plot([(ts[i]-threshold_min)/60000 for i in index], [temp[i] for i in index], label='Ax')
        axs[x_plot, y_plot].plot([(ts[i]-threshold_min)/60000 for i in index], [temp[i] for i in index], label='Ay')
        axs[x_plot, y_plot].plot([(ts[i]-threshold_min)/60000 for i in index], [temp[i] for i in index], label='Az')
