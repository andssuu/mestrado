...
class ListaAdjacencias():
    ...
    def nao_vizinhanca(self, u):
        return [v for v in self.V if v.valor not in self.lista[u.valor]]
...
class ListaAdjacenciasDigrafo(ListaAdjacencias):
    def __init__(self, V, E):
        self.V = [Vertice(v) for v in V[:]]
        self.E = E[:]
        self.lista = [[] for x in range(len(V))]
    ...
    def reverso(G):
        ...
        for e in E:
            e[0], e[1] = e[1], e[0]
        return ListaAdjacenciasDigrafo([v.valor for v in G.V], E)
